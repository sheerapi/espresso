...
namespace graphics::gl
{
	void GraphicDeviceGL::init()
	{
		...
		if (SDL_GL_SetSwapInterval(_window->getVsync() ? -1 : 0) == -1)
		{
			SDL_GL_SetSwapInterval(_window->getVsync() ? 1 : 0);
		}
        
		_vsync = _window->getVsync();
	...
	void GraphicDeviceGL::_setVsync()
	{
		if (_vsync == _window->getVsync())
		{
			return;
		}
