...
namespace core
{
	...
	public:
		...
		static void registerProcessor()
		{
			...
			processors.push_back(std::make_unique<T>());
		}
	...
	private:
		...
		inline static std::unordered_map<std::string, std::shared_ptr<Asset>> loadedAssets;
		inline static std::vector<std::unique_ptr<AssetProcessor>> processors;
